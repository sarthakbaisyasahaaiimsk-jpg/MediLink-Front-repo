...
import {
  ...
  BookUser, Plus, Trash2, UserPlus, UserMinus, ChevronDown,
  Check, FolderOpen, X, Pencil, MoreHorizontal
} from 'lucide-react';
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-2">
          <div className="space-y-1.5">
            ...
            <Input
              ...
              placeholder="e.g. Cardiology colleagues"
              maxLength={100}
            />
          </div>
          <div className="space-y-1.5">
      ...
      {groups.length === 0 ? (
        <div className="flex-1 flex flex-col items-center justify-center text-center py-8 px-4">
          ...
          <p className="text-xs text-slate-400 mt-1">
            Create a group and add doctors from the network
          </p>
        </div>
      ) : (
        <div className="space-y-1 overflow-y-auto flex-1">
          ...
          >
            ...
            <span className="text-xs text-slate-400">
              {[...new Set(groups.flatMap(g => g.members.map(m => m.user_id)))].length}
            </span>
          </button>
      ...
      {selectedGroup && (
        <div className="mt-4 border-t border-slate-100 pt-4">
          ...
          {selectedGroup.members.length === 0 ? (
            <p className="text-xs text-slate-400 px-1">No members yet. Add from the network.</p>
          ) : (
            <div className="space-y-1 max-h-48 overflow-y-auto">
              {selectedGroup.members.map(m => (
                <div key={m.user_id} className="flex items-center gap-2 px-1 py-1 rounded-lg hover:bg-slate-50 group/member">
                  ...
                  <button
                    onClick={() => onRemoveMember(selectedGroup.id, m.user_id)}
...
export default function Network() {
  ...
  const createGroupMutation = useMutation({
    mutationFn: ({ name, color }) => contactsApi.createGroup(name, color),
    onSuccess:  invalidate,
  });

  const updateGroupMutation = useMutation({
    mutationFn: ({ id, name, color }) => contactsApi.updateGroup(id, { name, color }),
    onSuccess:  invalidate,
  });

  const deleteGroupMutation = useMutation({
    mutationFn: (id) => contactsApi.deleteGroup(id),
    onSuccess:  () => {
      invalidate();
  ...
  const removeMemberMutation = useMutation({
    mutationFn: ({ groupId, userId }) => contactsApi.removeMember(groupId, userId),
    onSuccess:  invalidate,
  });
  ...
  const handleRenameGroup = (name, color) => {
    if (!editingGroup) return;
    updateGroupMutation.mutate({ id: editingGroup.id, name, color });
    setEditingGroup(null);
  };

  const handleDeleteGroup = (id) => {
    if (!window.confirm('Delete this group? Members will not be affected.')) return;
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <Button
                ...
                onClick={() => {
                  setContactsOpen(o => !o);
                  if (contactsOpen) setSelectedGroupId(null);
                }}
              >
        ...
        <div className="flex-1 min-w-0">
          {isLoading ? (
            ...
            <div className="text-center py-16">
              ...
              <p className="text-slate-500 mt-1">
                {contactsOpen
                  ? 'Add doctors from the full network view'
